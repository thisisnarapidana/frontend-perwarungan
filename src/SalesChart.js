...
function SalesChart() {
    ...
    const fetchDataFromDatabase = async () => {
        try {
            const response = await fetch(apiUrl + '/detail/detailed' + "/2024-01-02T08:17:02.635Z/2024-01-31T08:17:02.635Z", {
                method: 'GET',
                headers: {
    ...
    const modalData = (data) => {
        ...
        data.forEach((transaction) => {
            const total = transaction.detailed_transactions.reduce((acc, curr) => {
                if (curr.qty_stock_change > 0) return acc + curr.qty_stock_change * curr.item.cogs * -1; // berarti modal
                else return 0;
            }, 0);
    ...
    const untungKotorData = (data) => {
        ...
        data.forEach((transaction) => {
            const total = transaction.detailed_transactions.reduce((acc, curr) => {
                if (curr.qty_stock_change < 0) return acc + curr.qty_stock_change * curr.item.price * -1;
                else return 0
            }, 0);
    ...
    const untungBersihData = (data) => {
        ...
        data.forEach((transaction) => {
            const total = transaction.detailed_transactions.reduce((acc, curr) => {
                if (curr.qty_stock_change < 0) return acc + curr.qty_stock_change * (curr.item.price - curr.item.cogs) * -1;
                else return 0
            }, 0);
