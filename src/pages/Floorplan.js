...
const Floorplan = ({ rolee }) => {
  ...
  useEffect(() => {
    fetchData();
    // Add event listener to detect clicks on the floorplan background
  ...
  const handleBackgroundClick = (event) => {
    // Check if the click occurred outside of any table element
    const isClickedOnTable = event.target.closest(".table-container");
    if (!isClickedOnTable) {
      onDrag(draggedTableId, false);
    }
  };
  ...
  const handleDelete = async (table_id) => {
    try {
      setTables((prevTables) =>
        prevTables.filter((table) => table.table_id !== table_id),
      );
